# BoardTAU Database Redesign - Detailed Implementation Prompt

## Project Context
You are working on the BoardTAU platform, a boarding house management and booking system. The current database structure has several issues that need to be resolved to improve data integrity, performance, and maintainability.
...
## Prisma Schema Changes to Implement

### 1. Enums
Implement the following enums in Prisma:
```prisma
enum Role {
  USER
  ADMIN
...
Create a detailed data migration script to:
...
Please follow this plan strictly to ensure a successful database redesign.
