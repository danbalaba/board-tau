...
export function TransactionKPICards({ stats, total, isLoading }: TransactionKPICardsProps) {
  ...
  const successRate = total > 0 ? (stats.completedCount / total) * 100 : 0;
  ...
  const kpis = [
    { 
      ...
      trend: computeTrend(total, total * 0.95, false), // mock previous
      tooltip: {
        title: 'Total Transactions',
        description: 'Total number of payment attempts processed through the platform.',
        detail: 'Includes all statuses: Paid, Pending, and Failed.'
      }
    },
    { 
      label: 'Gross Revenue', 
      ...
      bg: 'bg-emerald-500/10', 
      chartColor: '#10b981', 
      trendData: generateTrendData(stats.totalAmount, !!isLoading),
      trend: computeTrend(stats.totalAmount, stats.totalAmount * 0.87, false), // mock previous
      tooltip: {
        title: 'Gross Revenue Volume',
        description: 'The total amount of money from successfully completed transactions.',
        detail: 'Represents only the completed bookings.'
      }
    },
    { 
      label: 'Success Rate', 
      ...
      bg: 'bg-amber-500/10', 
      chartColor: '#f59e0b', 
      trendData: generateTrendData(successRate, !!isLoading),
      trend: computeTrend(successRate, successRate * 0.99, false), // mock previous
      tooltip: {
        title: 'Payment Success Rate',
        description: 'Percentage of total transactions that were paid successfully.',
        detail: 'A high success rate indicates a healthy checkout flow.'
      }
    },
    { 
      label: 'Failed Ops', 
      ...
      bg: 'bg-rose-500/10', 
      chartColor: '#f43f5e', 
      trendData: generateTrendData(stats.failedCount, !!isLoading),
      trend: computeTrend(stats.failedCount, stats.failedCount * 1.02, true), // inverted logic
      tooltip: {
        title: 'Failed Transactions',
        description: 'Number of payments that were declined or failed to process.',
        detail: 'High numbers here indicate issues with user cards or the payment gateway.'
      }
    },
  ];
                      ...
                      <div className="h-20 w-full mt-6 -mx-6 mb-[-1.5rem]">
                        <ResponsiveContainer width="100%" height="100%">
                          <AreaChart data={stat.trendData}>
                            <defs>
                              <linearGradient id={`gradient-tx-${i}`} x1="0" y1="0" x2="0" y2="1">
