...
export function GovernanceCenter() {
  ...
  return (
    <PageContainer
      ...
      pageDescription="Administrative oversight and final authorization pipeline"
      pageHeaderAction={
        <div className="flex items-center gap-3">
          {total > 0 && (
             <span className="hidden md:flex items-center gap-1.5 bg-indigo-500/10 px-3 py-1.5 rounded-full border border-indigo-500/10 text-indigo-500 text-[10px] tracking-[0.2em] font-black italic">
          ...
          <Button
            ...
            size="icon"
            onClick={handleSync}
            className={cn("h-11 w-11 rounded-xl shadow-sm", (isLoading || isFetching) && "animate-spin")}
          >
          ...
          <Button
            variant="default"
            className="h-11 px-6 gap-2 font-black uppercase tracking-widest bg-indigo-600 hover:bg-indigo-700 shadow-lg shadow-indigo-600/20 text-white"
          >
            <IconFilter className="w-4 h-4" /> Pipeline Filters
          </Button>
        </div>
      }
    >
        ...
        <div className="space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-2">
            <div className="flex items-center gap-3 pl-4 border-l-[3px] border-indigo-500">
              <IconInbox className="w-5 h-5 text-indigo-500" />
              ...
              <div className="space-y-4 p-8 bg-card/40 rounded-[32px] border border-border/40 backdrop-blur-sm">
                ...
                <div className="flex items-center gap-6">
                  ...
                  <Button variant="outline" size="icon" className="h-12 w-12 rounded-2xl border-border/40 hover:bg-indigo-600 hover:text-white transition-all group">
                    <IconExternalLink className="w-5 h-5 group-hover:scale-110 transition-transform" />
                  </Button>
                </div>
              </div>
