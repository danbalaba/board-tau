...
import { MessageSquare, Star, RefreshCw, Search, LayoutGrid, List, Calendar, ChevronDown, Download, Check, Filter, X } from 'lucide-react';
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <p className="text-[11px] text-gray-500 dark:text-gray-400 font-bold uppercase tracking-[0.2em]">
                  Review and moderate student feedback, ratings, and tenant reviews across BoardTAU
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 mt-4 md:mt-0 ml-auto">
                  ...
                  return (
                    <DropdownMenuItem
                      ...
                      className={cn(
                        ...
                        isSelected
                          ? 'bg-amber-500 text-white data-[highlighted]:bg-amber-500 data-[highlighted]:text-white focus:bg-amber-500 focus:text-white hover:bg-amber-500 hover:text-white shadow-md shadow-amber-500/20'
                          : 'text-gray-600 dark:text-gray-300 data-[highlighted]:bg-amber-500/10 data-[highlighted]:text-amber-600 focus:bg-amber-500/10 focus:text-amber-600 hover:bg-amber-500/10 hover:text-amber-600'
                      )}
                    >
            ...
            {handleRefresh && (
              ...
              >
                <RefreshCw size={16} className={cn("text-gray-500 dark:text-gray-400 group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors", (isLoading || isFetching) && "animate-spin [animation-duration:2s]")} />
              </Button>
            )}
            ...
            {isSuperAdmin && onExport && (
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-48 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border-gray-100 dark:border-gray-800 rounded-2xl p-2 shadow-2xl">
                  <DropdownMenuItem onClick={() => onExport?.('PDF')} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl data-[highlighted]:bg-amber-500/10 data-[highlighted]:text-amber-600 hover:bg-amber-500/10 focus:bg-amber-500/10 focus:text-amber-600 hover:text-amber-600 transition-all text-xs font-bold text-gray-700 dark:text-gray-300 group cursor-pointer mb-1 outline-none">
                    <IconFileTypePdf className="w-4 h-4 text-red-500 group-hover:scale-110 transition-transform" />
                    PDF Document
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => onExport?.('EXCEL')} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl data-[highlighted]:bg-amber-500/10 data-[highlighted]:text-amber-600 hover:bg-amber-500/10 focus:bg-amber-500/10 focus:text-amber-600 hover:text-amber-600 transition-all text-xs font-bold text-gray-700 dark:text-gray-300 group cursor-pointer mb-1 outline-none">
                    <IconTable className="w-4 h-4 text-green-500 group-hover:scale-110 transition-transform" />
                    Excel
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => onExport?.('CSV')} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl data-[highlighted]:bg-amber-500/10 data-[highlighted]:text-amber-600 hover:bg-amber-500/10 focus:bg-amber-500/10 focus:text-amber-600 hover:text-amber-600 transition-all text-xs font-bold text-gray-700 dark:text-gray-300 group cursor-pointer outline-none">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center gap-4 pt-8 border-t border-gray-100 dark:border-gray-800">
          ...
          <div className="flex items-center gap-2 lg:ml-auto">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-2 px-4 py-3 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 text-[10px] font-black uppercase tracking-widest text-gray-500 hover:text-amber-500 transition-all shadow-sm h-12">
                  <span>Sort: {sortBy === 'newest' ? 'Newest' : sortBy === 'oldest' ? 'Oldest' : sortBy === 'rating_high' ? 'High Rating' : sortBy === 'rating_low' ? 'Low Rating' : 'Newest'}</span>
                  <ChevronDown size={14} className="opacity-50" />
                </button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-48 p-2 rounded-2xl shadow-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800">
                {[
