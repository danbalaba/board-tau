...
export function DeleteAttributeModal({
  ...
  const handleDelete = async () => {
    try {
      setLoading(true);
      const targetId = attribute.id || attribute._id;
      if (!targetId) {
      ...
      if (res.data?.success) {
        ...
        toast.error(res.data?.message || "Failed to delete dynamic attribute");
      }
    } catch (error: any) {
      toast.error(error.response?.data?.message || error.message || "Failed to delete dynamic attribute");
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      ...
      <div className="flex flex-col items-center text-center pt-2">
        ...
        <h3 className="text-xl font-black text-slate-900 dark:text-white mb-1.5 tracking-tight">
          {loading ? "Deleting Attribute..." : "Delete Dynamic Attribute?"}
        </h3>
        ...
        <p className="text-xs text-slate-600 dark:text-slate-400 mb-6 leading-relaxed">
          {loading ? (
            <span className="text-red-600 dark:text-red-400 font-bold">
              Permanently removing attribute... Please wait.
            </span>
          ) : (
            <>
              Are you sure you want to permanently delete <strong className="text-slate-900 dark:text-slate-100">{attribute.name}</strong>? This action cannot be undone and will remove it from all property filters and search wizards.
            </>
          )}
        </p>
        ...
        <div className="flex items-center gap-3 w-full">
          ...
          <Button
            ...
            disabled={loading}
            className="flex-1 py-5 rounded-xl bg-red-600 hover:bg-red-700 text-white font-black text-xs shadow-md shadow-red-600/20 transition-all cursor-pointer flex items-center justify-center gap-2"
          >
            {loading ? (
              <>
