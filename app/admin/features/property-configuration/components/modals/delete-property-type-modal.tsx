...
export function DeletePropertyTypeModal({
  propertyType,
  onCloseModal,
  onSuccess,
}: DeletePropertyTypeModalProps) {
  const [loading, setLoading] = useState(false);

  const handleDelete = async () => {
    try {
      setLoading(true);
      const res = await axios.delete(`/api/admin/property-types/${propertyType.id}`);
      
      if (res.data?.success) {
        toast.success(`Property type '${propertyType.name}' deleted successfully.`);
        // Call onSuccess with deleted ID AFTER backend deletion succeeds
        if (onSuccess) onSuccess(propertyType.id);
        onCloseModal?.();
      } else {
        toast.error(res.data?.error || "Failed to delete property type");
      }
    } catch (error: any) {
      toast.error(error.response?.data?.message || error.message || "Failed to delete property type");
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      ...
      <div className="flex flex-col items-center text-center pt-2">
        ...
        <h3 className="text-xl font-black text-slate-900 dark:text-white mb-1.5 tracking-tight">
          {loading ? "Deleting Property Category..." : "Delete Property Category?"}
        </h3>
        ...
        <p className="text-xs text-slate-600 dark:text-slate-400 mb-6 leading-relaxed">
          {loading ? (
            <span className="text-red-600 dark:text-red-400 font-bold">
              Deleting category and updating attached property records... Please wait.
            </span>
          ) : (
            <>
              Are you sure you want to permanently delete <strong className="text-slate-900 dark:text-slate-100">{propertyType.name}</strong>? Any attached listings will automatically transition to <span className="font-bold text-amber-600 dark:text-amber-400">UNPUBLISHED</span> status to prevent orphaned data.
            </>
          )}
        </p>
