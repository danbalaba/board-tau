...
export function LandlordDashboardQuickActions({ onViewProfile }: { onViewProfile: () => void }) {
  const quickActions = [
    ...
    },
    {
      ...
      description: 'Check your landlord profile',
    },
    {
        ...
        if (action.onClick) {
          return (
            <button
              ...
              className="group relative text-left bg-white dark:bg-gray-900 rounded-[28px] border border-gray-100 dark:border-gray-800 p-8 hover:shadow-2xl hover:border-primary/30 transition-all duration-500 overflow-hidden w-full"
            >
              {CardContent}
            </button>
          );
