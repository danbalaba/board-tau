...
export default function LandlordTopbar({ user: initialUser }: LandlordTopbarProps) {
  ...
  const isSettingsOpen = useLandlordProfileStore((state: any) => state.isSettingsOpen);
  const activeTab = useLandlordProfileStore((state: any) => state.activeTab);
  ...
  // Sync global store with initial server data
  useEffect(() => {
    if (!isInitialized && initialUser) {
      setUser(initialUser);
    }
  }, [isInitialized, initialUser, setUser]);
  
  const user = storeUser || initialUser;
  const isLoading = localLoading || !isInitialized;
