...
const CustomAmenityModal: React.FC<CustomAmenityModalProps> = ({
  ...
  const handleCustomAmenitySubmit = () => {
    if (customAmenityText.trim() && selectedRoomIndex !== -1) {
      ...
      const updatedRooms = JSON.parse(JSON.stringify(currentRooms));
      
      const customAmenity = `${customAmenityText.trim()}|${selectedIcon}`;

      if (!updatedRooms[selectedRoomIndex].amenities) {
        updatedRooms[selectedRoomIndex].amenities = [];
      }
  ...
  return (
    ...
    >
      <Modal.WindowHeader title="Add Custom Feature" onClose={handleClose} />
