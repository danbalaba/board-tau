...
import { Search, HelpCircle, Shield, Sparkles } from 'lucide-react';
...
const CustomFeatureModal: React.FC<CustomFeatureModalProps> = ({
  setValue,
  getValues,
  onClose
}) => {
  ...
  const filteredIcons = useMemo(() => {
    if (!debouncedQuery) {
      return ['Shield', 'Lock', 'Camera', 'Eye', 'Zap', 'Waves', 'Flame', 'Bell', 'Smartphone', 'Key', 'Activity', 'Fingerprint'];
    }
  ...
  const handleSubmit = () => {
    if (text.trim()) {
      const current = getValues('propertyConfig.features') || [];
      const newValue = `${text.trim()}|${selectedIcon}`;

      if (!current.includes(newValue)) {
        setValue('propertyConfig.features', [...current, newValue]);
      }
      handleClose();
    }
  };
  ...
  return (
    ...
    >
      <Modal.WindowHeader title="Custom Property Feature" onClose={handleClose} />
      <div className="p-8 space-y-8">
        <div className="relative group">
          <Input
            ...
            placeholder="e.g. 24/7 Gate Guard, Smart Lock System"
            required
            useStaticLabel
          />
          <div className="flex items-center gap-2 mt-2 ml-1">
            ...
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
              Highlight what makes your property secure and modern
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="relative">
            ...
            <input 
              ...
              placeholder="Search icons (e.g. 'Shield', 'Lock', 'Camera')..."
        ...
        <div className="flex items-center gap-3 pt-4 border-t border-gray-50 dark:border-gray-800">
          ...
          <Button type="button" onClick={handleSubmit} disabled={!text.trim()} className="flex-1 rounded-2xl uppercase text-[11px] font-black tracking-widest h-14 bg-primary text-white shadow-xl shadow-primary/20">Add Feature</Button>
        </div>
      </div>
    </motion.div>
  );
