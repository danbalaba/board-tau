...
import { Search, HelpCircle, AlertCircle, Sparkles } from 'lucide-react';
...
const CustomRuleModal: React.FC<CustomRuleModalProps> = ({
  setValue,
  getValues,
  onClose
}) => {
  ...
  const [selectedIcon, setSelectedIcon] = useState('AlertCircle');
  ...
  const filteredIcons = useMemo(() => {
    if (!debouncedQuery) {
      return ['AlertCircle', 'Ban', 'Clock', 'VolumeX', 'Baby', 'Dog', 'Smoking', 'Users', 'CameraOff', 'Trash2', 'LogOut', 'Info'];
    }
  ...
  const handleSubmit = () => {
    if (text.trim()) {
      const current = getValues('propertyConfig.rules') || [];
      const newValue = `${text.trim()}|${selectedIcon}`;

      if (!current.includes(newValue)) {
        setValue('propertyConfig.rules', [...current, newValue]);
      }
      handleClose();
    }
  };
  ...
  return (
    ...
    >
      <Modal.WindowHeader title="Custom House Rule" onClose={handleClose} />
      <div className="p-8 space-y-8">
        <div className="relative group">
          <Input
            ...
            placeholder="e.g. No visitors after 8 PM, Quiet hours start at 10 PM"
            required
            useStaticLabel
          />
        ...
        <div className="space-y-4">
          ...
          <div className="relative">
            ...
            <input 
              ...
              placeholder="Search icons (e.g. 'Clock', 'No', 'Alert')..."
        ...
        <div className="flex items-center gap-3 pt-4 border-t border-gray-50 dark:border-gray-800">
          ...
          <Button type="button" onClick={handleSubmit} disabled={!text.trim()} className="flex-1 rounded-2xl uppercase text-[11px] font-black tracking-widest h-14 bg-primary text-white shadow-xl shadow-primary/20">Add Rule</Button>
        </div>
      </div>
    </motion.div>
  );
