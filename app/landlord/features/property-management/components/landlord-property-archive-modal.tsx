...
interface LandlordPropertyArchiveModalProps {
  isOpen: boolean;
...
export function LandlordPropertyArchiveModal({
  isOpen,
  onClose,
  property,
  onConfirm,
  isLoading
}: LandlordPropertyArchiveModalProps) {
  const isClient = useIsClient();

  if (!property || !isClient) return null;

  const isArchived = (property as any).isArchived;
          ...
          >
            ...
            <div className="flex flex-col items-center text-center">
              <div className={cn(
                ...
                isArchived ? "bg-primary/10 dark:bg-primary/20 text-primary" : "bg-amber-50 dark:bg-amber-900/20 text-amber-500"
              )}>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-8 leading-relaxed font-medium">
                {isArchived 
                  ? `Are you sure you want to restore "${property.title}"? It will be moved back to your active portfolio.`
                  : `Are you sure you want to archive "${property.title}"? This will hide it from your active listings but keep it for your records.`
                }
              </p>
