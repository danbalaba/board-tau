...
const STEPS = [
  ...
  { id: 'config', title: 'Property Config', icon: Building2 },
  { id: 'location', title: 'Location', icon: Building2 },
  { id: 'images', title: 'Images', icon: Building2 },
  { id: 'rooms', title: 'Room Setup', icon: Building2 },
  { id: 'documents', title: 'Documents', icon: Building2 },
  { id: 'review', title: 'Review', icon: Building2 },
];

export default function LandlordPropertyEditorCreate() {
  ...
  const renderStep = () => {
    switch (currentStep) {
      case 0: return <PropertyBasicStep register={register} errors={errors} watch={watch} control={control} />;
      case 1: return <PropertyConfigStep register={register} errors={errors} watch={watch} control={control} getValues={getValues} />;
      case 2: return <LocationStep register={register} errors={errors} watch={watch} mapCenter={mapCenter} onLocationSelect={handleLocationSelect} onAddressAutoFill={handleAddressAutoFill} />;
      case 3: return <PropertyImagesStep register={register} errors={errors} watch={watch} control={control} getValues={getValues} setValue={setValue} clearErrors={clearErrors} onPropertyFilesChange={(files) => setPropertyFiles(files)} onRoomFilesChange={(index, files) => setRoomFiles(prev => ({ ...prev, [index]: files }))} />;
      case 4: 
        // Note: create useFieldArray needs methods.control which is provided by FormProvider
        return (
          <RoomConfigStep 
            register={register} 
            errors={errors} 
      ...
      case 5: return <DocumentsStep register={register} errors={errors} uploadedFiles={uploadedFiles} onFileUpload={handleFileUpload} />;
      case 6: return <ReviewStep watch={watch} onBack={handleBack} />;
      default: return null;
    }
  };
      ...
      <AnimatePresence>
        {!isMounted ? (
          <div className="flex-1 flex items-center justify-center py-60 bg-black/10 backdrop-blur-[2px]">
            <LoadingAnimation text="Synchronizing property creator..." size="large" />
          </div>
        ) : (
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: "easeOut" }}>
            <div className="relative overflow-hidden bg-gradient-to-br from-primary/10 via-primary/5 to-transparent p-6 rounded-2xl border border-primary/10 shadow-sm mb-12">
              <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  <div className="p-2.5 bg-white dark:bg-gray-800 rounded-xl shadow-lg text-primary"><Building2 size={20} /></div>
                  <div>
                    <h1 className="text-2xl font-black text-gray-900 dark:text-white leading-tight">New Listing</h1>
                    <p className="text-[11px] text-gray-500 font-bold uppercase tracking-wider">Expand your rental portfolio</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mb-12 relative">
                ...
                {STEPS.map((step, index) => {
                  ...
                  return (
                    <div key={step.id} className="flex flex-col items-center group cursor-pointer" onClick={() => index < currentStep && setCurrentStep(index)}>
                      <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300 border-2", isActive ? "bg-primary border-primary text-white shadow-lg shadow-primary/20 scale-105" : isCompleted ? "bg-green-500 border-green-500 text-white" : "bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-400 group-hover:border-primary/40")}>
              ...
              <div className="absolute top-6 left-0 w-full h-0.5 bg-gray-100 dark:bg-gray-800 -z-0">
                <motion.div className="h-full bg-primary" initial={{ width: '0%' }} animate={{ width: `${(currentStep / (STEPS.length - 1)) * 100}%` }} transition={{ duration: 0.5 }} />
              </div>
            </div>
                ...
                <div className="mt-10 pt-8 border-t border-gray-100 dark:border-gray-800">
                  <div className="grid grid-cols-2 sm:grid-cols-3 items-center gap-4">
                    <div className="flex justify-start">
                      {currentStep > 0 && <Button outline type="button" onClick={handleBack} disabled={isSubmitting} className="min-w-[140px] px-6 py-2.5 flex items-center justify-center gap-2 text-xs font-black uppercase tracking-widest transition-all"><ChevronLeft className="w-4 h-4" />Previous</Button>}
                    </div>
                    <div className="hidden sm:flex flex-col items-center">
                      <span className="text-[9px] font-black uppercase tracking-[0.2em] text-gray-400 mb-0.5">Progress</span>
                      <span className="text-[11px] font-black text-primary uppercase tracking-widest">Step {currentStep + 1} of {STEPS.length}</span>
                    </div>
                    <div className="flex justify-end">
                      {currentStep === STEPS.length - 1 ? (
                        <Button onClick={handleSubmit(onSubmit)} disabled={isSubmitting} className="min-w-[140px] px-8 py-2.5 text-xs font-black uppercase tracking-widest transition-all shadow-md shadow-primary/10">{isSubmitting ? <><Loader2 className="w-4 h-4 animate-spin mr-2" />Submitting...</> : 'Submit Listing'}</Button>
                      ) : (
                        <Button onClick={handleNext} className="min-w-[140px] px-8 py-2.5 flex items-center justify-center gap-2 text-xs font-black uppercase tracking-widest transition-all shadow-md shadow-primary/10">Next Step<ChevronRight className="w-4 h-4" /></Button>
                      )}
                    </div>
                  </div>
                </div>
