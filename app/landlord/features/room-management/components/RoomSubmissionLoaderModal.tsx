...
import { Check, Sparkles, DoorOpen, CloudUpload, ShieldCheck, Bed, Layers } from 'lucide-react';

import { preloadKerbyAssets } from '@/utils/imagePreloader';

interface RoomSubmissionLoaderModalProps {
  isOpen: boolean;
  progress?: number;
  stage?: 1 | 2 | 3 | 4;
...
const STAGES = [
  {
    id: 1,
    title: 'Validating Unit Specifications',
    subtitle: 'Kerby is auditing your unit name, layout, and monthly pricing...',
    mascotSrc: '/assets/mascot/kerby-landlord-blueprint.png',
    fallbackSrc: '/assets/mascot/kerby-500-mechanic.png',
    icon: DoorOpen,
    color: 'from-amber-500 to-orange-500',
    badge: 'Step 1 of 4: Spec Audit'
  },
  {
    id: 2,
    title: 'Checking Bed & Amenity Setup',
    subtitle: 'Verifying guest capacity, bed configurations, and in-unit amenities...',
    mascotSrc: '/assets/mascot/kerby-landlord-checklist.png',
    fallbackSrc: '/assets/mascot/kerby-casual-studying.png',
    icon: ShieldCheck,
    color: 'from-blue-500 to-indigo-500',
    badge: 'Step 2 of 4: Amenity Check'
  },
  {
    id: 3,
    title: 'Syncing Gallery Photos & Media',
    subtitle: 'Encrypting and saving unit photos to BoardTAU EdgeStore Cloud...',
    mascotSrc: '/assets/mascot/kerby-landlord-uploading.png',
    fallbackSrc: '/assets/mascot/kerby-uniform-driving.png',
    icon: CloudUpload,
    color: 'from-purple-500 to-pink-500',
    badge: 'Step 3 of 4: Media Upload'
  },
  {
    id: 4,
    title: 'Unit Published Successfully!',
    subtitle: 'Your room unit is now live and ready for TAU student inquiries!',
    ...
    badge: 'Step 4 of 4: Unit Live'
  }
];

const ROOM_LOADING_TIPS = [
  "Units with clear bed counts and guest capacities receive 2.5x more student inquiries!",
  "Air-conditioned rooms with private bathrooms are top-searched by TAU students during enrollment.",
  "Real-time room availability prevents double-booking and builds instant tenant trust.",
  "You can edit unit pricing, description, or photos anytime from your Room Management terminal."
];

export function RoomSubmissionLoaderModal({
  isOpen,
  ...
  unitName,
  onComplete
}: RoomSubmissionLoaderModalProps) {
  ...
  useEffect(() => {
    ...
    // Auto-smooth timer if controlled internally (relaxed 7.5 seconds total)
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setTipIdx((prev) => (prev + 1) % ROOM_LOADING_TIPS.length);
    }, 3000);
  ...
  const currentStageObj = actionType === 'edit' ? {
    ...rawStageObj,
    title: currentStageNum === 1 ? 'Auditing Updated Unit Specs' : currentStageNum === 2 ? 'Verifying Amenity & Layout Sync' : currentStageNum === 3 ? 'Syncing Photos & Media Updates' : 'Unit Details Synchronized!',
    subtitle: currentStageNum === 1 ? `Kerby is auditing updated specs for ${unitName || 'your unit'}...` : currentStageNum === 2 ? 'Checking bed setup, capacity limits, and amenity options...' : currentStageNum === 3 ? 'Encrypting and updating room photo gallery...' : 'Unit details updated and synchronized with live property listing!',
    badge: currentStageNum === 1 ? 'Step 1: Spec Sync' : currentStageNum === 2 ? 'Step 2: Amenity Sync' : currentStageNum === 3 ? 'Step 3: Gallery Sync' : 'Step 4: Sync Complete'
  } : rawStageObj;
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h2 className="text-xs sm:text-sm font-black uppercase tracking-wider text-slate-900 dark:text-white truncate">
              {unitName ? `Unit: ${unitName}` : 'BoardTAU Unit Management'}
            </h2>
            <span className="text-[9px] sm:text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest block truncate">
              {actionType === 'edit' ? 'Unit Synchronization Pipeline' : 'Unit Publishing Pipeline'}
            </span>
          </div>
        </div>
      ...
      <div className="w-full max-w-2xl my-auto py-6 sm:py-8 flex flex-col items-center text-center z-10 space-y-4 sm:space-y-6">
        ...
        <div className="w-full max-w-lg space-y-2.5 pt-2">
          <div className="flex items-center justify-between text-xs font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">
            <span>{actionType === 'edit' ? 'Synchronization Progress' : 'Publishing Progress'}</span>
      ...
      <div className="w-full max-w-2xl z-10">
        <div className="p-4 sm:p-5 rounded-3xl bg-white/95 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800/80 text-left flex items-start gap-4 shadow-xl shadow-slate-200/60 dark:shadow-none backdrop-blur-md">
          ...
          <div className="flex-1 min-w-0">
            <span className="text-[10px] font-black text-amber-600 dark:text-amber-400 uppercase tracking-widest block mb-0.5">Unit Management Tip</span>
            <AnimatePresence mode="wait">
              <motion.p
                key={tipIdx}
