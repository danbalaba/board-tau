...
const Toggle: FC<ToggleProps> = ({ children, id, className }) => {
  ...
  return <>{cloneElement(children as React.ReactElement<{ onClick: (e: React.MouseEvent<HTMLButtonElement>) => void }>, { onClick: handleClick })}</>;
};
      ...
      {!openId ? null : (
        <motion.ul
          ...
          style={{
           ...
           className={cn(
             `absolute top-[110%]  w-max min-w-[170px] bg-white dark:bg-gray-800 rounded-sm z-[9999] text-[12px] overflow-hidden`,
             className,
