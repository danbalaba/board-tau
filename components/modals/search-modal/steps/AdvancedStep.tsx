...
export default function AdvancedStep({
  ...
  const [dynamicAttributes, setDynamicAttributes] = useState<any[]>(() => {
    const syncData = getTaxonomyDataSync("FEATURE");
  ...
  const [isLoading, setIsLoading] = useState<boolean>(() => {
    return !getTaxonomyDataSync("FEATURE");
  });
  ...
  const activeSubGroups = useMemo(() => {
    ...
    return groups.filter((sg: any) => {
      const matchingAttrs = dynamicAttributes.filter((attr: any) => {
        ...
        if (!rawTypes || rawTypes.length === 0) return true;
  ...
  const currentItems = dynamicAttributes
    ...
    .map((attr) => {
      ...
      return {
        ...
        name: attr.name,
        icon: IconComp,
        desc: attr.description || `Custom ${attr.name} security feature.`,
      };
    });

  const currentConfig = useMemo(() => {
    let pose: KerbyPose = "pointing";
    if (currentSubGroupKey === "DISASTER_PREP") pose = "thinking";
    if (currentSubGroupKey === "HEALTH_SANITATION") pose = "loving";

    return {
      title: currentSubGroup?.title || `Step 9-${subStep + 1}: ${label}`,
      subtitle: currentSubGroup?.subtitle || `Which ${label.toLowerCase()} features do you prefer for your ${propName}?`,
      speech: `Which ${label.toLowerCase()} features do you prefer for your ${propName}?`,
      pose,
      items: currentItems,
    };
  ...
  const SUB_STEP_TABS = useMemo(() => {
    return activeSubGroups.map((sg, idx) => ({
  ...
  const hasCurrentSubStepSelections = currentConfig.items.some((item: any) =>
    advancedSelected.includes(item.id)
  );

  const handleClearSubStep = () => {
    currentConfig.items.forEach((item: any) => {
      if (advancedSelected.includes(item.id)) {
        toggleMulti("advanced", item.id);
      }
    });
  };
    ...
    >
      ...
      {isLoading ? (
        ...
        <Heading
          ...
          helpText="Select priority safety, gate access, and disaster preparedness features."
          rightAction={
      ...
      {isLoading ? (
        <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar w-full">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-7 w-24 rounded-full bg-slate-200/70 dark:bg-slate-800/70 shrink-0" />
          ))}
        </div>
      ) : (
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {currentConfig.items.map((opt: any) => {
              const isSelected = advancedSelected.includes(opt.id);
              const Icon = opt.icon;

              return (
                <div
                  key={opt.id}
                  onClick={() => toggleMulti("advanced", opt.id)}
