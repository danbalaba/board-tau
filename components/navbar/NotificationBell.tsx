...
const NotificationBell: React.FC<NotificationBellProps> = ({ user }) => {
  ...
  // Use a cleanup ref so we can remove the listener when the dropdown unmounts
  const cleanupRef = useRef<(() => void) | null>(null);

  // Callback ref: fires every time the dropdown panel mounts or unmounts
  // (unlike useEffect + useRef which runs once on component mount before the panel exists)
  const panelRef = useCallback((node: HTMLDivElement | null) => {
    ...
    const preventScroll = (e: WheelEvent) => {
      const scrollableArea = node.querySelector<HTMLElement>('.notification-scroll-area');
      // If cursor is over the scrollable list, allow normal scroll
      if (scrollableArea && scrollableArea.contains(e.target as Node)) {
        return;
      }
      // Header / filter pills area — block page scroll
      e.preventDefault();
    };

    node.addEventListener('wheel', preventScroll, { passive: false });
    cleanupRef.current = () => node.removeEventListener('wheel', preventScroll);
  }, []);
      ...
      <Menu>
        ...
        <Menu.List className="hidden md:block shadow-[0_12px_40px_rgba(0,0,0,0.12)] dark:shadow-[0_12px_40px_rgba(0,0,0,0.4)] rounded-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 w-[360px] md:w-[400px] overflow-hidden !right-0 transform translate-x-1/4 md:translate-x-0 mt-2">
          <div ref={panelRef}>
            ...
            {/* Content */}
            <div className="notification-scroll-area max-h-[60vh] overflow-y-auto overscroll-contain custom-scrollbar p-2">
